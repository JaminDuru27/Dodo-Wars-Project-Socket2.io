import * as CHARACTERS from './characters/export.js'
...
export function Player(socket, io, info){
    const res ={
        ...
        load(Room, Game){  
            this.killed = (killer)=>{
                killer.score += Game.modes.mode.scoreinc
                // killer.killedarray.forEach(cb=>cb())
                console.log(`player ${killer.id} just killed someone, score:${killer.score}`)
            }
            this.killed.bind(this)
            this.character = CHARACTERS[info.character](this, socket, io, Game)
